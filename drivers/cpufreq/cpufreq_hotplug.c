...
#define DEF_CPU_UP_THRESHOLD        (40)
#define MICRO_CPU_UP_THRESHOLD      (35)
...
static struct dbs_tuners {
    ...
    unsigned int cpu_hotplug_disable1;
} dbs_tuners_ins = {
...
show_one(cpu_hotplug_disable1, cpu_hotplug_disable1);
...
static ssize_t store_cpu_hotplug_disable1(struct kobject *a, struct attribute *b,
				    const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	dbs_tuners_ins.cpu_hotplug_disable1 = input;
	return count;
}
...
define_one_global_rw(cpu_hotplug_disable1);
...
{
	mutex_lock(&hp_mutex);
	dbs_tuners_ins.cpu_hotplug_disable1 = disable;
...
static void hp_work_handler(struct work_struct *work)
{
	...
	{
		if (!dbs_tuners_ins.cpu_hotplug_disable1)
		{
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	printk("cpufreq_gov_dbs_init: dbs_tuners_ins.cpu_hotplug_disable1 = %d\n", dbs_tuners_ins.cpu_hotplug_disable1);
	#endif 
